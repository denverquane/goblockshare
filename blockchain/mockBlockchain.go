...
func (chain *BlockChain) AddMockTransactions() {
	torr, fileErr := common.MakeTorrentFileFromFile(1000, "READE.md", "readme")
	for i := 0; i < 3; i++ {
		...
		worked, err := chain.AddTransaction(signed, "test addr", nil)
		if !worked {
		...
		for i := 0; i < 10; i++ {
			...
			worked, err = chain.AddTransaction(signed2, "test addr", nil)
			if !worked {
				log.Println(err.Error())
			}
		...
		if fileErr == nil {
			...
			worked, err = chain.AddTransaction(signed3, "test addr", nil)
			if !worked {
				log.Println(err.Error())
			}
			for chain.IsProcessing() {
			...
			worked, err = chain.AddTransaction(signed4, "test addr", nil)
			if !worked {
				log.Println(err.Error())
			}
		}
		...
		btt5 := common.SignableTransaction{address4.ConvertToOriginInfo(), trans5, common.SET_ALIAS, nil, nil, ""}
		signed5 := btt5.SignAndSetTxID(&address4.PrivateKey)
		log.Println("Gonna broadcast " + signed5.TxID + " to blockchains")
		worked, err = chain.AddTransaction(signed5, "test addr", nil)
		if !worked {
			log.Println(err.Error())
		}
	}
